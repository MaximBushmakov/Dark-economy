...
namespace WorldSystem
{
    public class AllLocalEvents
    {
        ...
        public void SetEvents()
        {
            ...
            DictionaryOfBadVillageEvents.Add(0, new LocalEvent("Кража в доме", BadVillageEventName, "Пока вы мирно спали в доме, кто-то украл у кас деньги", new List<int>() { }, new List<string>(), new List<LocalEventEffect>() { new LocalEventEffect(KapitalLocalEffectName, -100) }));
            BadVillageEventsStartEvents.Add(0);
            DictionaryOfStoryEvents = new Dictionary<int, LocalEvent>();
            DictionaryOfBadVillageEvents.Add(0, new LocalEvent("Начало истории", StoryEventName, "Йохан, чудесное имя. Ваше имя. Я всем сердцем желаю, чтобы вы наслаждались даным вам телом в этом мире. Вы хотите знать, кто вы и где находитесь?", new List<int>() {2, 1}, new List<string>(){"Хочу", "Мне не интересно"}, new List<LocalEventEffect>()));
            DictionaryOfBadVillageEvents.Add(1, new LocalEvent("Приятной игры", StoryEventName, "Тогда, настало время начать ваше приключение. Да сопутсвует вам удача.", new List<int>() {2, 1}, new List<string>(){"Хочу", "Мне не интересно"}, new List<LocalEventEffect>()));
            DictionaryOfBadVillageEvents.Add(2, new LocalEvent("История мира", StoryEventName, "Этот мир можно назвать площадкой игры для богов, полем для проверки навыков их послушников. Но какое дело до этого вам, простому человеку? Для вас, это просто радной мир, мир злата и магии, мир эльфов, людей, гномов и монстров.", new List<int>() {1, 3}, new List<string>(){"Назад", "Далее"}, new List<LocalEventEffect>()));
            DictionaryOfBadVillageEvents.Add(2, new LocalEvent("Кто я?", StoryEventName, "Ваше тело, вы, Йохан, один из людей этого мира, проживающих в деревне на севере, практически у самой границы с землями монстров. После смерти отца, когда ваш старший брат Марк получил землянной надел от отца, вы решили стать торговцем", new List<int>() {2, 4}, new List<string>(){"Назад", "Далее"}, new List<LocalEventEffect>()));
            DictionaryOfBadVillageEvents.Add(2, new LocalEvent("Путь", StoryEventName, "Всё что у вас есть, небольшой запас капитала, немного товаров, что можно продать мельнику и верный друг - осёл по имени Суслик. Станете ли вы владельцем новой великой гильдии, или же умрёте с голову, потратив все деньги? А может сдадитесь и станете работать на брата? Решат лишь ваши действи и щепотка удачи.", new List<int>() {3, 0, 1}, new List<string>(){"Назад", "Расскажи с начала", "Я всё понял"}, new List<LocalEventEffect>()));
        }
